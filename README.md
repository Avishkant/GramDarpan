...
![Home screenshot](client/src/assets/Gemini_Generated_Image_x0pzwxx0pzwxx0pz.png)

If you deploy the repo and build the client, the backend will serve the built SPA from `client/dist`.
...
- Single-service deploy: the backend can serve the built client from `client/dist` (the repo includes code to do this). Alternatively the client can be hosted separately as a static site.
...
Security & secrets
------------------
- Do not commit `.env` files or secrets. The repo now includes `.env.example` files and `.gitignore` rules to prevent accidental commits.
- If secrets were committed at any point, rotate them immediately (MongoDB password, DATA_GOV_API_KEY, ADMIN_TOKEN).
- Use Render/host environment settings to store secrets.
...
Screenshots & assets
--------------------
Screenshot used above is in `client/src/assets/Gemini_Generated_Image_x0pzwxx0pzwxx0pz.png`.
...
- Implement Redis-based shared caching for low-latency cross-instance reads.
- Add a small admin endpoint to invalidate caches post-ETL.

Contact
-------
Repo: https://github.com/Avishkant/GramDarpan
